...
class LmsProvisioningGatewayDb implements LmsProvisioningGateway
{
    public function activate(array $ctx, bool $dryRun): array
    {
        ...
        if (!$idActive) return ['ok' => false, 'code' => 'PREREQ_MISSING', 'message' => 'Identity activation is required for LMS provisioning.'];

        $ref = 'LMS-' . strtoupper(substr(hash('sha256', 'lms|' . $studentId), 0, 12));
        return OperationalMarkerService::activate($studentId, 'lms', $ref, [
    ...
    {
        $studentId = (int) ($ctx['student_id'] ?? 0);
        return OperationalMarkerService::rollback($studentId, 'lms', $rollbackMeta);
    }

    public function reconcile(array $ctx): array
    {
        $studentId = (int) ($ctx['student_id'] ?? 0);
        return OperationalMarkerService::reconcile($studentId, 'lms');
    }

    public function health(): array
    {
        return ['ok' => true, 'adapter' => 'db_stub', 'service' => 'lms'];
    }
}
