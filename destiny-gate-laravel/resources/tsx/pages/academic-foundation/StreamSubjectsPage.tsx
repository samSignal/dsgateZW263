...
export default function StreamSubjectsPage() {
  ...
  return <div><PageHeader title="Stream Subjects" subtitle="Assign compulsory and optional subjects offered by each stream" /><Card style={{ marginBottom: 20 }}><CardHeader title="Assign Subject" /><CardBody><div className="grid grid-cols-1 gap-3 md:grid-cols-3"><FormGroup label="Year"><Select value={form.academic_year_id} onChange={e => setForm(f => ({ ...f, academic_year_id: e.target.value }))}>{(meta.academic_years ?? []).map((y: any) => <option key={y.id} value={y.id}>{y.name}</option>)}</Select></FormGroup><FormGroup label="Term"><Select value={form.term_id} onChange={e => setForm(f => ({ ...f, term_id: e.target.value }))}>{(meta.terms ?? []).map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}</Select></FormGroup><FormGroup label="Form"><Select value={form.form_id} onChange={e => setForm(f => ({ ...f, form_id: e.target.value, stream_id: '' }))}>{(meta.forms ?? []).map((x: any) => <option key={x.id} value={x.id}>{x.name}</option>)}</Select></FormGroup><FormGroup label="Stream"><Select value={form.stream_id} onChange={e => setForm(f => ({ ...f, stream_id: e.target.value }))}>{streams.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></FormGroup><FormGroup label="Subject"><Select value={form.subject_id} onChange={e => setForm(f => ({ ...f, subject_id: e.target.value }))}>{(meta.subjects ?? []).map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></FormGroup><label className="mt-7 flex items-center gap-2 text-sm"><input type="checkbox" checked={form.is_compulsory} onChange={e => setForm(f => ({ ...f, is_compulsory: e.target.checked }))} /> Compulsory</label></div><Btn loading={save.isPending} onClick={() => save.mutate()}>Assign Subject</Btn></CardBody></Card><Card><CardHeader title="Assigned Stream Subjects" /><Table headers={['Stream', 'Subject', 'Type', 'Year', 'Term']}>{data.map(r => <tr key={r.id}><Td>{r.form_name} {r.stream_name}</Td><Td>{r.subject_name}<br /><span className="text-xs text-slate-500">{r.subject_code}</span></Td><Td><Badge variant={r.is_compulsory ? 'green' : 'blue'}>{r.is_compulsory ? 'Compulsory' : 'Optional'}</Badge></Td><Td>{r.academic_year_name}</Td><Td>{r.term_name}</Td></tr>)}</Table></Card></div>;
}
