      ...
      <Card style={{ marginBottom: 20 }}>
        ...
        <CardBody>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <FormGroup label="Stream"><Select value={form.stream_id} onChange={e => setForm(f => ({ ...f, stream_id: e.target.value }))}><option value="">Select</option>{filteredStreams.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></FormGroup>
