...
type Item = { id: number; shop_category_id: number; category_name: string; item_name: string; item_code: string; description: string | null; unit_price: number; quantity_in_stock: number; reorder_level: number; is_active: boolean; is_low_stock: number };
const empty = { shop_category_id: '', item_name: '', item_code: '', description: '', unit_price: '', quantity_in_stock: '0', reorder_level: '0', is_active: true };
...
export default function ShopItemsPage() {
  ...
  const openEdit = (i: Item) => {
    setEditing(i);
    setForm({ shop_category_id: String(i.shop_category_id), item_name: i.item_name, item_code: i.item_code, description: i.description ?? '', unit_price: String(i.unit_price), quantity_in_stock: String(i.quantity_in_stock), reorder_level: String(i.reorder_level), is_active: i.is_active });
    setError('');
    setOpen(true);
  };
    ...
    <div>
      ...
      <Card>
        {items.length === 0 ? <Empty message="No shop items found." /> : (
          <Table headers={['Item', 'Category', 'Price', 'Stock', 'Status', 'Actions']}>
            {items.map(i => (
              <tr key={i.id}>
                ...
                <Td><div className="flex flex-wrap gap-2"><Btn size="sm" variant="outline" onClick={() => openEdit(i)}>Edit</Btn><Btn size="sm" variant="outline" onClick={() => toggle.mutate(i)}>{i.is_active ? 'Deactivate' : 'Activate'}</Btn><Btn size="sm" variant="danger" onClick={async () => { if (await confirmDelete(i.item_name)) del.mutate(i.id); }}>Delete</Btn></div></Td>
              </tr>
            ))}
          </Table>
        )}
